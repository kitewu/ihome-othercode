...
static uint8 init_air[] = {0x02, 0x00, 0x2E, 0x08, 0x00};
static uint8 air_cmd[] = {0x00, 0x00, 0x08,0x08, 0x00};
...
static void initIrDA(void)
{
	init_air[4] = GetXOR(init_air[0],init_air[1],init_air[2],init_air[3]);
	HalUARTWrite(SERIAL_APP_PORT, (uint8*)init_air, sizeof(init_air));
}
	...
	case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据
          ...
          num1 = CheckSum((uint8*)receiveData,receiveData->Len); 
            if((receiveData->Header==0x40)&&(receiveData->Verify==num1)) //校验包头包尾
            {
              ...
              if(receiveData->NodeID == IrDA) //ID校验
              {
                Send_Air_Cmd(receiveData->Command[0],receiveData->Command[1]);
                Rsp.Data[0] = receiveData->Command[0];
                Rsp.Data[1] = receiveData->Command[1];
...
*说    明 ： 给红外模块发送指令
*****************************************************************************************/
static void Send_Air_Cmd(uint8 cmd, uint8 data)
{
	air_cmd[0] = cmd;
	air_cmd[1] = data;
