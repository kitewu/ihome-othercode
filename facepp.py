...
def sendMsg(name) : 
	...
	sock.connect(('115.159.127.79', 5678))
	sock.send(name)
	sock.close()
	
if __name__ == 	'__main__' :
	result = commands.getstatusoutput("ls /mnt/sda1/python/*.jpg")
	if result[0] >= 0:
		file = result[1].split('\n')[0]
		API_KEY = 'b68340946ae8e52bd5ae95442b994405'
		API_SECRET = 'znG4oojNL1l-Ic3GV5_F_FuTQQC6KCiw'
		api = API(API_KEY, API_SECRET)
		res = api.recognition.identify(group_name = 'ihome', img = File(file))
		print res
		if len(res['face']) != 0 :
			max = 0
			name = ''
			for person in res['face'][0]['candidate'] :
				if person['confidence'] >= max :
					max = person['confidence']
					name = person['person_name']
			if max > 50 :
				name = '1/3/ihome_001/' + name + "来访"
			else :
				name = '1/3/ihome_001/有人来访'
		else :
			name = '1/3/ihome_001/有人来访'
		sendMsg(name)